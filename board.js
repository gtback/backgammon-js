...
// board.js is both a drop-in browser <script> (it exposes Diagram / BoardStyle on
// the global object) and a CommonJS module for node:test. The whole file is wrapped
// in a guarded IIFE so that re-injecting the script into a single realm — e.g. Anki
// reloading a card inside one long-lived webview — is a harmless no-op instead of a
// `class Diagram has already been declared` SyntaxError.
;(function (global) {
  // Idempotent: if a previous load already installed the API, do nothing.
  if (global.Diagram && global.BoardStyle) return

  // roundRect (used for the frame, cube, and dice) is missing on older WebKit /
  // Chromium builds — including the QtWebEngine bundled with older Anki. Polyfill it
  // with arcTo so those clients render instead of throwing. All internal callers pass
  // a single numeric radius.
...
`
  ...
  // Public API. Exposed on the global object for browser <script> use (Diagram /
  // BoardStyle and friends become available to later scripts in the page) and as
  // module.exports for node:test, which also reaches the pure helpers.
  const _api = {
    Diagram,
    BoardStyle,
    mergeOptions,
  ...
  for (const name in _api) global[name] = _api[name]
